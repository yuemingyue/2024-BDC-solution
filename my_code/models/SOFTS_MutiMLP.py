...
from my_code.layers.SelfAttention_Family import CBAMAttention
...
class SOFTS_CNN(nn.Module):
    ...
    def forecast(self, x_enc):
        ...
        _, _, N = x_enc.shape
        #split
        era_in,temp_wind=x_enc[:,:,:36],x_enc[:,:,-2:]
        era_in = era_in.reshape(-1,168,4,9).permute(0,3,2,1)
        era_in = self.era_attn(era_in).permute(0,3,1,2).reshape(-1,168,36)
        x_enc = torch.cat([era_in,temp_wind],axis=-1)
        enc_out = self.enc_embedding(x_enc, None)
        enc_out, attns = self.encoder(enc_out, attn_mask=None)
        enc_out = enc_out.permute(0,2,1).unsqueeze(-1)
...
class SOFTS_ATTN_TEMP(nn.Module):
    def __init__(self,seq_len,enc_in,enc_d_model,enc_d_core,pred_len,d_model,dropout,use_norm,d_core,d_ff,activation,e_layers,n_heads=1):
        super(SOFTS_ATTN_TEMP, self).__init__()
        self.seq_len = seq_len
        self.pred_len = pred_len
        self.d_model = d_model
        # Embedding
        ...
        self.time_attn = CBAMAttention(d_model,8,15)
        self.enc_embedding = DataEmbedding_inverted(seq_len,d_model,dropout)
        self.use_norm = use_norm
        # Encoder
        self.encoder = Encoder(
...
class DAIN_Layer(nn.Module):
    def __init__(self, mode='adaptive_avg', mean_lr=0.001, gate_lr=0.001, scale_lr=0.001, input_dim=120):
