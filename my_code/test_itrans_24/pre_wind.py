...
model=itransformer(seq_len=168,pred_len=24,output_attention=0,dropout=0.2,d_model=64,n_heads=1,d_ff=64,activation='gelu',e_layers=1).to(device)
#OPTIMIZER
lr=0.003
#LOSS
loss_fn=nn.MSELoss()
    ...
    for step,(batch_x,batch_y) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Training epoch {epo+1}", leave=True):
        ...
        if step%500==0 and step!=0:
            ...
            if score_sum<best_score:
                ...
                torch.save(model.state_dict(),'/home/mw/project/best_model/best_test_24_itrans_model_wind.pt')
    
    loss_sum=sum(train_loss)/len(train_loss)
    mse_sum = sum(train_mse)/len(train_mse)
    score_sum = sum(train_score)/len(train_score)
    print(f'now epoch{epo}:loss:{loss_sum},mse:{mse_sum},score:{score_sum}')
    # torch.save(model.state_dict(),f'/home/mw/project/best_model/test_24_itrans_model_wind_epo_{epo}.pt')
    print(f'now epoch{epo}:loss:{loss_sum},mse:{mse_sum},score:{score_sum}')