...
class CNNLSTMDecoder(nn.Module):
    def __init__(self, d_model, pred_len, dropout, kernel_size, num_lstm_layers, d_ff):
        ...
        self.lstm = nn.LSTM(input_size=64, hidden_size=d_model, num_layers=num_lstm_layers, batch_first=True, dropout=dropout,bidirectional=True)
...
for num_fold,(train_idx,valid_idx) in enumerate(skf.split(all_index,all_index)):
    ...
    # 初始化模型 配置
    model =itransformer(seq_len=168,pred_len=72,output_attention=0,dropout=0.1,d_model=64,n_heads=1,d_ff=128,activation='gelu',e_layers=1).to(device)
    ...
    for epo in range(epochs):
        ...
        for step,(batch_x,batch_y) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Training fold {num_fold+1} epoch {epo+1}", leave=True):
            ...
            if loss_mode=='mse':
                ...
                loss = (torch.fft.rfft(batch_out, dim=1) - torch.fft.rfft(batch_y, dim=1)).abs().mean()*0.4+0.6*loss_fn(batch_out,batch_y)
            elif loss_mode=='mix':
            ...
            elif loss_mode == 'dif_loss':
                loss = loss_fn(batch_out,batch_y)+loss_fn(batch_out[:, 1:, :] - batch_out[:, :-1, :],batch_y[:, 1:, :] - batch_y[:, :-1, :])
            ...
            if step%500==0 and step!=0:
                ...
                if score_sum<best_score:
                    ...
                    torch.save(model.state_dict(),f'/home/mw/project/best_model/best_test_72_itrans1_model_wind_{num_fold+1}.pt')
    ...
    # 保存fold 级别模型
    torch.save(model.state_dict(),f'/home/mw/project/best_model/test_72_itrans1_model_wind_fold_{num_fold+1}.pt')
    break
    ...
    # torch.save(new_state_dict, f'swa_wind_model_{num_fold+1}.pt')
