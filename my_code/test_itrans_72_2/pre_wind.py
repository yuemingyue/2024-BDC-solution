...
# 导入模型
from models.itransformer_wind import itransformer_lstm
# ema 训练 动态学习率
...
for num_fold,(train_idx,valid_idx) in enumerate(skf.split(all_index,all_index)):
    ...
    # 初始化模型 配置
    model = itransformer_lstm(seq_len=168,pred_len=72,output_attention=0,dropout=0.1,d_model=144,n_heads=1,d_ff=128,activation='gelu',e_layers=1).to(device)
    #AMP
        ...
        for step,(batch_x,batch_y) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Training fold {num_fold+1} epoch {epo+1}", leave=True):
            ...
            if step%500==0 and step!=0:
                ...
                if score_sum<best_score:
                    ...
                    torch.save(model.state_dict(),f'/home/mw/project/best_model/best_test_72_itrans2_model_wind_{num_fold+1}.pt')
