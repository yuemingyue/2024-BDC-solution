...
data_path='wind.npy'
exter_path='temp.npy'
dataset=Dataset_Meteorology(root_path,data_path,exter_path,[168,1,24],features='MS',data_fliter=False)
...
model=SOFTS_ATTN(seq_len=168,enc_in=38,enc_d_model=64,enc_d_core=24,pred_len=24,d_model=128,dropout=.05,use_norm=True,d_core=48,d_ff=160,activation='gelu',e_layers=1).to(device)
#OPTIMIZER
lr=4e-3
wd=5e-7
grad_accum_step=1
    ...
    for step,(batch_x,batch_y) in tqdm(enumerate(loader), total=len(loader), desc=f"epoch {epo+1}", leave=True):
        ...
        if step%500==0 and step!=0:
            ...
            if epo_score_sum<bst_score:
                ...
                torch.save(model.state_dict(),'/home/mw/project/best_model/best_attn_soft.pt')
    scheduler.step()
    epo_loss_sum=sum(epo_loss)/len(epo_loss)
    epo_score_sum=sum(epo_score)/len(epo_score)
    torch.save(model.state_dict(),f'/home/mw/project/best_model/best_attn_soft_epo_{epo}.pt')
    print(f'now epoch{epo},now step{step}:mse_loss:{epo_loss_sum},score:{epo_score_sum}')
