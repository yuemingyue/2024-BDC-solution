...
data_path='temp.npy'
exter_path='wind.npy'
dataset=Dataset_Fuse(root_path,data_path,exter_path,[168,1,48],features='MS')
...
model=SOFTS_MLP(seq_len=168,pred_len=48,d_model=96,dropout=.05,use_norm=True,d_core=64,d_ff=128,activation='gelu',e_layers=1).to(device)
#OPTIMIZER
lr=1e-2
wd=1e-5
    ...
    for step,(batch_x,batch_y) in tqdm(enumerate(loader), total=len(loader), desc=f"Training epoch {epo+1}", leave=True):
        ...
        if step%500==0 and step!=0:
            ...
            if epo_loss_sum<bst_loss:
                ...
                torch.save(model.state_dict(),'/home/mw/project/best_model/best_test_48_softs_model_temp.pt')
    epo_loss_sum=sum(epo_loss)/len(epo_loss)
    print(f'now epoch{epo}:mse_loss:{epo_loss_sum}')
    torch.save(model.state_dict(),f'/home/mw/project/best_model/test_48_softs_model_temp_epo_{epo}.pt')
