...
from models.Dain_SOFTS_WIND import Dain_SOFTS_WIND
...
model=Dain_SOFTS_WIND(seq_len=168,enc_in=38,enc_d_model=64,enc_d_core=24,pred_len=72,d_model=168+72,dropout=.2,use_norm=True,d_core=72,d_ff=256,activation='gelu',e_layers=1).to(device)
#OPTIMIZER
lr=7e-3
wd=5e-5
grad_accum_step=1
...
for num_fold,(train_idx,valid_idx) in enumerate(skf.split(all_index,all_index)):
    ...
    train_loader = DataLoader(dataset,batch_size=bs,shuffle=shuffle_flag,num_workers=30)
    ...
    for epo in range(epochs):
        ...
        for step,(batch_x,batch_y) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Training fold {num_fold+1} epoch {epo+1}", leave=True):
            ...
            if step%500==0 and step!=0:
                ...
                if score_sum<best_score:
                    ...
                    #torch.save(model.state_dict(),f'softs_wind_kfold_checkpoint/best_softs_wind_model_fold_{num_fold+1}_epo_{epo}.pt')  
        if epo>=1:
            print("swa_model")
        ...
        torch.save(model.state_dict(),f'/home/mw/project/best_model/softs_wind_model_fold_{num_fold+1}_epo_{epo}.pt')
    # fold 级别指标
    print(f'now fold {num_fold+1} mse:{np.mean(train_mse_fold)},score:{np.mean(train_score_fold)}' )
    #保存swa模型
    # torch.optim.swa_utils.update_bn(train_loader, swa_model, device=device)
    #torch.save(swa_model.state_dict(), f'softs_wind_kfold_checkpoint/swa_softs_wind_model_fold_{num_fold+1}_epo_{epo}.pt')